...
* Б. Многопоточный веб-сервер (2 балла) && Г. Ограничение потоков сервера (3 балла) 
    ...
    + Происзодит следующее: Запрашивается `html` в котором есть картинка, но на неё не хватает процессов для обработки и она догружается уже через следующие 10 секунд. (Браузер почему то отправляет лишнии `http` пустые запросы после)
